...
next: "Clear the 06-19 re-audit's open FAILs: IDOR in investigate/[id]/threads (carry-over — never executed), accessibility (silent civic forms, unlabelled close buttons, unassociated form labels), DB-infra (TLS verify on, tracked migrations + CREATE EXTENSION vector, pgvector ANN index). Then: RLS runtime enforcement, magic_codes purge on account deletion, branch protection gating deploy on CI, ws override to restore CI audit-gate=high"
testing: null
pinned: true
shipped:
  - date: 2026-06-19
