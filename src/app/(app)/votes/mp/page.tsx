        ...
        <div className="space-y-2">
          {mps.map((mp) => (
            <Link
              key={mp.id}
              href={`/votes/mp/${mp.id}`}
              className="group flex items-center gap-4 rounded-xl border px-4 py-3 transition-all duration-150 hover:bg-white/[0.04] hover:border-white/[0.10]"
              style={{
                borderColor: 'rgba(255,255,255,0.06)',
                backgroundColor: 'rgba(255,255,255,0.015)',
              }}
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-neutral-200 leading-tight truncate">
                  {mp.name}
                </p>
